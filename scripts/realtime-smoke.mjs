  ...
  return new Promise(resolve => {
    ...
    const req = http.get(url, res => {
      ...
      res.on('end', () => {
        ...
        const appOk = body.includes('Creature Sandbox') && body.includes('/assets/') && !body.includes('/@vite/client');
...
async function verifyOfflineReload(page) {
  ...
  assert.ok(
    registrationState.cacheKeys.some(key => key.startsWith('creature-sandbox-')),
    'service worker should create Creature Sandbox caches'
  );
  ...
  await waitForPageCondition(
    page,
    () => typeof window.__creatureSmoke?.perfBudget === 'function',
    'smoke hooks after online SW reload'
  );
  await waitForPageCondition(
    page,
    () => {
    ...
    'sprite assets cached after online SW reload',
    15000
  );
...
try {
  ...
  const sampledFps = samples
    .map(sample => Number(sample.timing?.sampledFps || sample.fps || 0))
  ...
  assert.ok(
    worstFrameTime <= maxFrameTimeMs,
    `sampled frame time should stay <= ${maxFrameTimeMs}ms; got ${worstFrameTime.toFixed(1)}ms`
  );
