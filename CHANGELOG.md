...
- **Verification:** Badge renders in all 3 branches and absent without threat (DOM-stub check); live screenshot shows HUNTED on a followed creature with zero staging; lint clean; tests green (190/0, 60/0, e2e 1/0, save + migration); smoke:menus 34/34; proof:release GREEN on the exact tree (one attempt tripped the known intermittent main-lane startup CLS at 0.1985; standalone + back-to-back lanes green, retry green — same pre-existing race documented 2026-09-03, none of this change is visible at startup: drawer/panel hidden, no selection → no badge). Shipped below; production results below.
