...
| Memory/learning | Y | Y | Y | Y | Selected-creature panel now explains current drive plus strongest remembered places. |
...
| Watch Mode controls | Y | Y | Y | Partial | Browser smoke verifies toggle and watch-strip controls; watch/god UI state is included in runtime metadata. |
...
| Save/load schema versioning + migrations | Y | N | Y | Y | Regression covers zero values, older fixture, memory, props, food bites, active events, and runtime metadata providers. |
| Performance guardrails (throttling, spatial partitioning) | Y | Partial | Y | Partial | Browser smoke asserts bounded sprite cache/population, and `npm run check:bundle` enforces post-build JS chunk budgets. |
