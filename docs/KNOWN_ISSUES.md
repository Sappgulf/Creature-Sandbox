...
1. **Camera world bounds still allow a 16px overscroll**
   - **Severity:** Low
   - **Description:** Camera clamping uses a 16px margin beyond world edges (down from 80px). Creatures flush against the border can still clip a few pixels after an aggressive pan.
...
4. **Overlay stacking now uses `--z-*` tokens; keep new UI on those rungs**
   - **Severity:** Low
...
5. **Skip-link and ripple timing now share the token set; leftover one-offs may remain**
   - **Severity:** Low
