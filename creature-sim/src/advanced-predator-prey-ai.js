...
export class AdvancedPredatorPreyAI {
   ...
   */
  static chaseStrategy(predator, prey, world, dt) {
   ...
   */
  static interceptStrategy(predator, prey, world) {
    // Calculate interception point
    const preyVelocity = Math.sqrt(prey.vx ** 2 + prey.vy ** 2);
    const predatorSpeed = (predator.genes?.speed ?? 1) * 50;
   ...
   */
  static patienceStrategy(predator, prey, world) {
    // Follow at distance
   ...
   */
  static selectEvasionStrategy(prey, predator, allPredators, world) {
    const distance = Math.sqrt(
    ...
    const preySpeed = prey.genes?.speed ?? 1;
    const predatorSpeed = predator.genes?.speed ?? 1;
    const panicLevel = prey.emotions?.fear ?? 0;
   ...
   */
  static zigzagEvasion(prey, predator, dt) {
    if (!prey.zigzagState) {
      prey.zigzagState = {
   ...
   */
  static hideEvasion(prey, predator, world) {
   ...
   */
  static fleeEvasion(prey, predator, world) {
    const awayAngle = Math.atan2(prey.y - predator.y, prey.x - predator.x);
   ...
   */
  static freezeEvasion(prey, predator) {
    return {
      x: prey.x,
      y: prey.y,
      strategy: 'freeze',
