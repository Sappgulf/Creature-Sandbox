...
function describeDirector(directorSnapshot = null) {
  const headline =
    ...
    null;
  const body =
    directorSnapshot?.body || directorSnapshot?.guidance?.body || directorSnapshot?.nextAction?.description || null;
...
export function buildAccessibilitySummary({
  ...
  const creatures = Array.isArray(world?.creatures)
