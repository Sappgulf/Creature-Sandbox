...
See the full [docs/RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md). The CI release-smoke workflow is intentionally absent (removed per user request); the canonical local gate is `npm run proof:release`, with `GOD/scripts/god-shadow.sh` covering lint/test/build/bundle only.
...
- **Barrel exports**: Several `creature-sim/src/*/index.js` barrels exist (`core`, `game`, `input`, `render`, `ui`, `platform`). New code and refactors should import through them where practical. We are gradually migrating hot paths.
- **Worker is default**: `creature-sim/src/simulation-proxy.js` + `creature-sim/src/worker-simulation.js` are the primary runtime. Main-thread fallback must stay green.
- **Save schema**: Versioned migrations live in `creature-sim/src/save-migration.js`. Never break roundtrips without a migration.
